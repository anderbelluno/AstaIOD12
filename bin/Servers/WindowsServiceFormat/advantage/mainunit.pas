...
type
  TAstaIOServiceMainForm = class(TAstaIOServiceForm)
...
function AstaIOServiceMainForm: TAstaIOServiceMainForm;

implementation

uses SocketDM, AstaIOUtil, AstaIOAdvantageSupplementDM, dm,adodb;

{$R *.DFM}

function AstaIOServiceMainForm: TAstaIOServiceMainForm;
begin
  result := TAstaIOServiceMainForm(AstaIOServiceUtils.AstaIOServiceForm);
end;

{ TAstaIOServiceForm }

procedure TAstaIOServiceMainForm.FormCreate(Sender: TObject);
begin
  IpAddressEdit.Text := GetThePCSIPAddress;
  RemoteTabSheet.TabVisible := UseRemoteControl;
end;

procedure TAstaIOServiceMainForm.Log(Msg: string);
begin
...
procedure TAstaIOServiceMainForm.Initialize(FromService: Boolean);
begin
...
function TAstaIOServiceMainForm.ConnectedClients: Boolean;
begin
  result := (ServerDM <> nil) and (ServerDm.ServerWire.UserList.Count > 0);
end;


procedure TAstaIOServiceMainForm.miCloseClick(Sender: TObject);
begin
...
procedure TAstaIOServiceMainForm.ApplyButtonClick(Sender: TObject);
begin
  ServerDM.ToggleServerWireActive;
end;

procedure TAstaIOServiceMainForm.LogCheckBoxClick(Sender: TObject);
begin
  if ServerDM <> nil then
    ServerDM.DoLogging := LogCheckBox.Checked;
end;

procedure TAstaIOServiceMainForm.im_mainClick(Sender: TObject);
begin
...
procedure TAstaIOServiceMainForm.DatabaseSetupButtonClick(Sender: TObject);
begin
  if ServerDM.ConnectDataBase then ServerDM.WriteSettings;
end;

procedure TAstaIOServiceMainForm.StartTheRemoteControl(StopIt: Boolean);
begin
...
procedure TAstaIOServiceMainForm.RemoteControlCheckBoxClick(Sender: TObject);
begin
...
procedure TAstaIOServiceMainForm.PropertiesClick(Sender: TObject);
begin
  ShowModal;
  ServerDM.WriteSettings;
end;

procedure TAstaIOServiceMainForm.EncryptCheckBoxClick(Sender: TObject);
begin
