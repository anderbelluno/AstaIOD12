...
uses dm, mainUnit,AstaIOAdvantageSupplementDM,Dialogs;
...
begin
  ...
  begin
    ServerWire.Port := StrToIntDef(AstaIOServiceMainForm.PortEdit.Text,
      9050);
    DoLogging := AstaIOServiceMainForm.LogCheckBox.checked; 
  ...
  begin
    ...
    AstaIOServiceMainForm.Caption := 'AstaIO Advantage Server serving on ' +
    ...
    AstaIOServiceMainForm.ServerDataLabel.Caption :=AstaIOAdvantageDBPluginDM.FConnectionString;
    ...
    ServerWire.RecordServerActivity(nil, 'Using ' + AstaIOAdvantageDBPluginDM.FConnectionString);
  end
  else
  begin
    AstaIOServiceMainForm.Caption := 'AstaIO Advantage Server NOT Active';
...
begin
  ...
  begin
    ...
    with AstaIOAdvantageDBPluginDM do
      begin
        FConnectionstring:=Reg.ReadString('Server', 'Alias','');
        result := VisualsetDatabase(List);
      end;
  end;
  if result then
  begin
    with AstaIOAdvantageDBPluginDM do begin
      ServerWire.RecordServerActivity(nil, 'Alias : ' + FConnectionString);
      Reg.WriteString('Server', 'Alias', FConnectionString);
   end;
 end;
end;
...
begin
  ...
  try
   Alias:=Reg.ReadString('Server','Alias','ADTDemoData');
   if InputQuery('Alias', 'Please Enter the Advantage Alias',Alias) then begin
      AstaIOAdvantageDBPluginDM.FConnectionString:=Alias;
      Reg.WriteString('Server', 'Alias', Alias);
...
begin
  ...
  with TAstaDataModule(DM).AdsConnection do begin
    //set up  your connection string here
    AliasName:=AstaIOAdvantageDBPluginDM.FConnectionString;
    isConnected := True;
  end;
...
begin
  ...
  try
    ...
    AstaIOAdvantageDBPluginDM.WriteDatabaseSettings(Reg);
  finally
    Reg.Free;
  end;
end;
