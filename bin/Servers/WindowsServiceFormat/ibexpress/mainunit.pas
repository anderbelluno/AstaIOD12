...
type
  TAstaIOIBXServiceForm = class(TAstaIOServiceForm)
...
function AstaIOIBXServiceForm: TAstaIOIBXServiceForm;

implementation

uses SocketDM, AstaIOUtil, AstaIOIBXSupplementDM, dm;

{$R *.DFM}

function AstaIOIBXServiceForm: TAstaIOIBXServiceForm;
begin
  result := TAstaIOIBXServiceForm(AstaIOServiceUtils.AstaIOServiceForm);
end;

{ TAstaIOIBXServiceForm }

procedure TAstaIOIBXServiceForm.FormCreate(Sender: TObject);
begin
  IpAddressEdit.Text := GetThePCSIPAddress;
  RemoteTabSheet.TabVisible := UseRemoteControl;
end;

procedure TAstaIOIBXServiceForm.Log(Msg: string);
begin
...
procedure TAstaIOIBXServiceForm.Initialize(FromService: Boolean);
begin
...
function TAstaIOIBXServiceForm.ConnectedClients: Boolean;
begin
  result := (ServerDM <> nil) and (ServerDm.ServerWire.UserList.Count > 0);
end;


procedure TAstaIOIBXServiceForm.miCloseClick(Sender: TObject);
begin
...
procedure TAstaIOIBXServiceForm.ApplyButtonClick(Sender: TObject);
begin
  ServerDM.ToggleServerWireActive;
end;

procedure TAstaIOIBXServiceForm.LogCheckBoxClick(Sender: TObject);
begin
  if ServerDM <> nil then
    ServerDM.DoLogging := LogCheckBox.Checked;
end;

procedure TAstaIOIBXServiceForm.im_mainClick(Sender: TObject);
begin
...
procedure TAstaIOIBXServiceForm.DatabaseSetupButtonClick(Sender: TObject);
begin
  if ServerDM.ConnectDataBase then ServerDM.WriteSettings;
end;

procedure TAstaIOIBXServiceForm.StartTheRemoteControl(StopIt: Boolean);
begin
...
procedure TAstaIOIBXServiceForm.RemoteControlCheckBoxClick(Sender: TObject);
begin
...
procedure TAstaIOIBXServiceForm.PropertiesClick(Sender: TObject);
begin
  ShowModal;
  ServerDM.WriteSettings;
end;

procedure TAstaIOIBXServiceForm.EncryptCheckBoxClick(Sender: TObject);
begin
