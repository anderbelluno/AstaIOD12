...
type
  Tf_main = class(TForm)
    ...
    m_log: TMemo;
    procedure ServerWireShowServerMessage(Sender, Client: TObject;
      LogMsg: string);
    ...
    procedure ServerWireAddDataModule(Sender: TObject; DM: TComponent;
      DMList: TList; SessionName: String);
...
uses dm_main,
     dm2, //additional datamodule for examples
     IBDatabase,
     IBStoredProc,
     IBCustomDataSet,
     IBQuery;
...
procedure Tf_main.ServerWireShowServerMessage(Sender, Client: TObject;
  LogMsg: string);
begin
  LogIt(LogMsg);
end;
...
//  if not Serversocket.Enabled then ServerSocket.Enabled:=True;
...
procedure Tf_main.ServerWireAddDataModule(Sender: TObject; DM: TComponent;
  DMList: TList; SessionName: String);
var
Extra:TAstaExtraDataModule;
i:Integer;
begin
 extra:=TAstaExtraDataModule.create(nil);
 //now loop through and use the database connection from the main datamodule
 for i:=0 to Extra.ComponentCount-1 do
  if Extra.Components[i] is TIBquery then
   with Extra.Components[i] AS TIBQuery do begin
    Database:=TAstaDataModule(dm).DataBase;
    Transaction:=TAstaDataModule(dm).Transaction;
  end;  
  DMList.Add(Extra);
end;
