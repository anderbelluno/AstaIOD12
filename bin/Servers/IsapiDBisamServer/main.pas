...
uses
  ...
  AstaIOStringClientWire,StringDM,Dbisam_main;
...
type
  ...
  public
    { Public declarations }
    FAstaDbisamDataModule:TAstaDbisamDataModule;
    FAstaStringDataModule:TAstaStringDataModule;
  end;
...
procedure TAstaModule.SendError(Response: TWebResponse; Error: word);
begin
...
{$IFDEF DEBUG}
procedure TAstaModule.WriteToLog(S: string);
var
  F: TextFile;
begin
...
procedure TAstaModule.SendDefaultPage(Response: TWebResponse);
begin
...
procedure TAstaModule.AstaModuleAstaAction(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
var
  Server: string;
  Port: word;
...
begin
  FAstaDbisamDataModule:=TAstaDbisamDataModule.Create(Self);
  FAstaStringDataModule:=TAstaStringDataModule.Create(self);
  Client.ServerWire:=FAstaStringDataModule.Server;
...
procedure TAstaModule.ProcessAstaIORequest(Request: TWebRequest;Response: TWebResponse);
var
  S: string;
begin
  WriteToLog('Starting to process Asta IO request');
  try
  Client.sendString(request.content);
  s:=TAstaIOStringServerWire(Client.ServerWire).Data;
  Response.StatusCode := 200;
