...
uses
  ...
  AstaIOSocketServer,AstaIOConst, DBCtrls;
...
begin
  ConnectToServers;
  ServerWire.Port:=StrToInt(PortEdit.Text);
  ServerWire.Active:=True;

  if ServerWire.Active then Caption:='Connected on port ' + PortEdit.Text;
...
procedure TAstaIOAnchorServerServiceForm.im_mainClick(Sender: TObject);
begin
...
//    ServerWire.TrustedAddresses.Text := AstaServerServiceForm.TrustedAddressesMemo.Text ;
  ...
  if ServerWire.Active then begin
    ...
    AstaIOAnchorServerServiceForm.Caption                 := 'ASTA Server serving on  port ' + IntToStr(ServerWire.Port);
    ...
    AstaIOAnchorServerServiceForm.Caption                := 'ASTA Server NOT Active';
