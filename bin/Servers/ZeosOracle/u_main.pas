{###########################################################
  ...
  This server needs the package "Zeos DBO".
  You find this in: http://www.zeoslib.org

  Any doubts, problems or suggestions, contact me please:
...
uses
  ...
  DB, Buttons, StdCtrls, ExtCtrls, ComCtrls, AstaIOMessagePacker,
  AstaIOUserList, AstaIODBConst, AstaIOCustomDataSet, AstaIOParamList,
  AstaIOMetaData, AstaIOOracleInfo, AstaIODataBasePlugin, AstaIOServerWire,
  AstaIOSocketServer, AstaIOConst, ZOraSqlTr, ZQuery, ZOraSqlQuery;
...
begin
  AstaDataModule.Database.Database := e_database.Text;
  AstaDataModule.Database.Login:=e_username.Text;
...
begin
  ...
  TZOraSqlQuery(SQLDataSet).ExecSql;
  RowsAffected := TZOraSqlQuery(SQLDataSet).RowsAffected;
end;
...
begin
  ...
  TZOraSQLTransact(Transaction).StartTransaction;
end;
...
begin
  ...
  TZOraSQLTransact(Transaction).Commit;
end;
...
procedure Tf_main.DataBasePluginSetProviderParams(Sender: TObject;
  ...
  ClientParams: TParams);
begin
  TZOraSqlQuery(DataSet).Params.Assign(ClientParams);
end;

procedure Tf_main.DataBasePluginCreateProviderParamsEvent(Sender: TObject;
  var Params: TParams; DataSet: TDataSet);
begin
  Params.Assign(TZOraSqlQuery(DataSet).Params);
end;
...
procedure Tf_main.ServerWireAssignPersisentSession(Sender: TObject;
  ...
  ParamsForClient: TAstaParamList; var Verified: Boolean);
begin
  if U.ParamList.FindParam('Packets')=nil then
  begin
  ...
  with TAstaDataModule(DatabaseSession).Database do
  begin
    Database := e_database.Text;
    Login:=e_username.Text;
    Password:=e_password.Text;
...
procedure Tf_main.DataBasePluginAutoIncrementFetch(Sender: TObject;
  ...
  AutoIncFieldName: String; var AutoIncrementValue: Integer);
begin
  with TZOraSqlQuery(SQLDataSet) do
  begin
...
begin
  ...
  begin
    case AdbAction of
      tdbSelect:begin
                  ADBComponent:=TZOraSqlQuery.Create(nil);
                  TZOraSqlQuery(ADBComponent).DataBase:=TAstaDataModule(U.DatabaseSession).Database;
                  TZOraSqlQuery(ADBComponent).Transaction:=TAstaDataModule(U.DatabaseSession).Transaction;
                end;
    end
  end
  else
...
i:integer;
begin
  with  TZOraSqlQuery(ADataSet) do
  begin
    SQL.text:=SQLString;
