...
type
  ...
  TAstaServerCodedMessageEvent = procedure(Sender: TObject; UserRecord: TUserRecord; MsgID: integer; Msg: AnsiString) of object;
  ...
  TAstaServerEncryptionEvent = procedure(Sender: Tobject; UserRecord: TUserRecord; var Data: AnsiString; EncryptIt: Boolean) of object;
  TAstaServerCompressionEvent = procedure(Sender: Tobject; UserRecord: TUserRecord; var Data: AnsiString; CompressIt: Boolean) of object;
  ...
  TAstaServerWriteMessageEvent = procedure(Sender: TObject; UserRecord: TUserRecord; S: AnsiString) of object;
  ...
  TIAstaServerTransportInterface = interface
    ...
    procedure ReceiveString(Client: TObject; S: AnsiString);
    procedure InternalSendString(UserRecord: TUserRecord; S: AnsiString);
...
{$ENDIF}
  ...
  protected
    ...
    procedure ReceiveString(Client: TObject; S: AnsiString); virtual;
    ...
    Function OpenEnvelope(UserRecord: TUserRecord; var TheData: AnsiString):Boolean;
    procedure SealEnvelope(UserRecord: TUserRecord; var TheData: AnsiString); virtual;
    ...
    procedure DoSendMessage(UserRecord: TUserRecord; S: AnsiString); virtual;
    procedure DoDecrypt(UserRecord: TUserRecord; var TheData: AnsiString); virtual;
    procedure DoEncrypt(UserRecord: TUserRecord; var TheData: AnsiString); virtual;
    //Database
    ...
    function FindPlugin(var S: AnsiString; var PluginData: pointer; var
...
{$IFDEF AstaDES}
  ...
  public
    ...
    procedure DoProviderBroadCast(Origin: TUserRecord; BroadcastMaterial: AnsiString; Provider: TComponent);
...
{$ENDIF}
    ...
    procedure SendString(user: TuserRecord; S: AnsiString); virtual;
    ...
    procedure InternalSendString(UserRecord: TUserRecord; S: AnsiString); virtual;
    ...
    procedure DoCodedMessage(U: TUserRecord; Msgid: Integer; Msg: AnsiString); virtual;
    ...
    procedure BroadcastCodedMessage(Originator: TUserRecord; Msgid: Integer; const Msg: AnsiString);
    ...
    procedure SendCodedMessage(U: TUserRecord; Msgid: Integer; const Msg: AnsiString);
    ...
    Procedure SetAESKeyStrings(Const InKey,OutKey:AnsiString);
    {$ENDIF}
    {$IFDEF AstaDES}
    procedure SetDESKey(AKey: PDESKey; KeyType: DesKeyType);
    Procedure SetDESStringKey(Const AKey:AnsiString);
    {$ENDIF}
...
begin
  ...
  begin
    If U.DatabaseSession <> Nil Then //<<<<<<<<<< Insert !!! jn - 07/19/2015
...
procedure TCustomAstaServerWire.SendCodedMessage(U: TUserRecord; Msgid: Integer; const Msg: AnsiString);
begin
  SendString(U, ServerMessageToString(ATCodedMessage, 0,[Msgid, Msg]));
end;

procedure TCustomAstaServerWire.BroadcastCodedMessage(Originator: TUserRecord; Msgid: Integer; const Msg: AnsiString);
var
  I: Integer;
  S: AnsiString;
  User: TUserRecord;
  ...
  begin
    ...
    if (User <> Originator) and (not User.IsLogger) then
      ...
      begin
        JavaPlugin := FindJavaPlugin();
        if JavaPlugin <> nil then
          JavaPlugin.JavaBroadcastParamList(User, MsgID, 'CodedMessage', String(Msg));
      end
      else
        SendString(User, S);
  end;
...
procedure TCustomAstaServerWire.DoSendMessage(UserRecord: TUserRecord; S: AnsiString);
begin
  if assigned(FMessageWriteEvent) then FMessageWriteEvent(Self, UserRecord, S);
end;

procedure TCustomAstaServerWire.SendString(User: TUserRecord; S: AnsiString);
begin
...
procedure TCustomAstaServerWire.DoProviderBroadCast(Origin: TUserRecord; BroadcastMaterial: AnsiString; Provider: TComponent);
begin
...
begin
  ...
  if Reader.Count > 1 then
    Data := AnsiString(Reader.ReadString(1))
  else
    Data := '';
...
function TCustomAstaServerWire.DoKeysExchange(U: TUserRecord;
  var Step: Integer; var Data: AnsiString): Boolean;
var
        ...
        begin
          ...
          try
            if Length(Data) > 0 then
              InStream.Write(Pointer(Data)^, Length(Data));
            InStream.Position := 0;
            RSADecrypt(FRSAKey, InStream, TempStream);
            if Step = 1 then
              begin
                SetString(Str, PAnsiChar(TempStream.Memory), TempStream.Size);
                U.UserKey := string(Str);
              end
            else
              begin
                {$ifdef AstaDES}
...
begin
  ...
  if FLogClientList.Indexof(U) < 0 then begin
    SendCodedMessage(U, adminLogUserList, AnsiString(UserListAsStringList));
...
function TCustomAstaServerWire.OpenEnvelope(UserRecord: TUserRecord; var TheData: AnsiString): Boolean;
begin
...
procedure TCustomAstaServerWire.SealEnvelope(UserRecord: TUserRecord; var TheData: AnsiString);
begin
  case FCompression of
    acNoCompression: ; //do nothing
...
procedure TCustomAstaServerWire.ReceiveString(Client: TObject; S: AnsiString);
type
  PInteger = ^Integer;
      ...
      tuDisconnect: begin
          for i := 0 to FLogClientList.Count - 1 do
            if U = FLogClientList[i] then FLogClientList.Delete(FLogClientList.Indexof(U)) else
              SendCodedMessage(TUserRecord(FLogClientList[i]), adminLogDisconnect, AnsiString(U.UserInfo));
        end;
      tuLoginSuccess: begin
          for i := 0 to FLogClientList.Count - 1 do
            SendCodedMessage(TUserRecord(FLogClientList[i]), adminLoginSuccess, AnsiString(U.UserInfo));
        end;
    end;
  end;
{$ENDIF}
...
procedure TCustomAstaServerWire.DoCodedMessage(U: TUserRecord; Msgid: Integer; Msg: AnsiString);
begin
    ...
    for i := 0 to FLogClientList.count - 1 do
      if (U <> nil) and (FLogClientList.Indexof(U) < 0) then begin
        if IsValid(U.TheClient) then
          SendCodedMessage(TUserRecord(FLogClientList[i]), adminLogCodeBase + UserDefined, AnsiString(Msg + ' ->' + RemoteAddressAndPort(U.TheClient)))
        else
          SendCodedMessage(TUserRecord(FLogClientList[i]), adminLogCodeBase + UserDefined, AnsiString('[ServerMsg]' + Msg));
      end;
  end;
{$ENDIF}
end;
...
procedure TCustomAstaServerWire.InternalSendString(UserRecord: TUserRecord; S: AnsiString);
begin
...
{$IFDEF AstaAES}
Procedure TCustomAstaServerWire.SetAESKeyStrings(Const InKey,OutKey:AnsiString);
var
AESKey:TAesKey;
...
function TCustomAstaServerWire.AESEncrypt(Data: AnsiString): AnsiString;
var InStr,
...
function TCustomAstaServerWire.AESDecrypt(Data: AnsiString): AnsiString;
var InStr,
  OutStr: TMemoryStream;
  Count: integer;
...
{$IFDEF AstaDES}
Procedure TCustomAstaServerWire.SetDESStringKey(Const AKey:AnsiString);
begin
...
function TCustomAstaServerWire.DESEncrypt(Data: AnsiString): AnsiString;
var InStr,
  OutStr: TMemoryStream;
  Count: integer;
begin
...
function TCustomAstaServerWire.DESDecrypt(Data: AnsiString): AnsiString;
var InStr,
  OutStr: TMemoryStream;
  Count: integer;
  NewHash, OrigHash: TMessageDigest128;
...
function TCustomAstaServerWire.FindPlugin(var S: AnsiString; var PluginData:
