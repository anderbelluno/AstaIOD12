...
{**********************************************************************}
{}
{ $Log:  10211: AstaIOKylixWorkBenchFrm.pas 
{
...
type
  ...
  TKylixWorkBenchForm = class(TForm)
    tlb_main: TToolBar;
...
var
  KylixWorkBenchForm: TKylixWorkBenchForm;
...
procedure TKylixWorkBenchForm.a_sql_cutExecute(Sender: TObject);
begin
  m_sql.CutToClipboard;
end;

procedure TKylixWorkBenchForm.a_sql_copyExecute(Sender: TObject);
begin
  m_sql.CopyToClipboard;
end;

procedure TKylixWorkBenchForm.a_sql_pasteExecute(Sender: TObject);
begin
  m_sql.PasteFromClipboard;
end;

procedure TKylixWorkBenchForm.a_sql_deleteExecute(Sender: TObject);
begin
  m_sql.ClearSelection;
end;

procedure TKylixWorkBenchForm.a_sql_clearExecute(Sender: TObject);
begin
  m_sql.Clear;
end;

procedure TKylixWorkBenchForm.a_sql_select_allExecute(Sender: TObject);
begin
  m_sql.SelectAll;
end;

procedure TKylixWorkBenchForm.a_sql_saveExecute(Sender: TObject);
begin
  if not SaveDialog.Execute then exit;
  m_sql.Lines.SaveToFile(SaveDialog.FileName);
end;

procedure TKylixWorkBenchForm.a_sql_openExecute(Sender: TObject);
begin
  if not OpenDialog.Execute then exit;
  m_sql.Lines.LoadFromFile(OpenDialog.FileName);
end;

procedure TKylixWorkBenchForm.a_sql_runExecute(Sender: TObject);
begin
...
procedure TKylixWorkBenchForm.a_populateExecute(Sender: TObject);
...
procedure TKylixWorkBenchForm.a_exitExecute(Sender: TObject);
begin
  // Need to do more here....
  Close;
end;

procedure TKylixWorkBenchForm.FormCreate(Sender: TObject);
begin
...
procedure TKylixWorkBenchForm.FormDestroy(Sender: TObject);
begin
  SQLParser.Free;
  NoSQLFields.Free;
  PrimeFields.Free;
...
procedure TKylixWorkBenchForm.FormActivate(Sender: TObject);
begin
  ts_properties.Visible:=False;
  ts_properties.Enabled:=False;
  tv_main.Execute;
end;

procedure TKylixWorkBenchForm.AssignFields;
var i    :Integer;
begin
...
procedure TKylixWorkBenchForm.q_dataAfterOpen(DataSet: TDataSet);
begin
  sb_data.Panels[0].Text:=IntToStr(q_data.RecordCount) + ' Rows';
end;

procedure TKylixWorkBenchForm.a_apply_fieldsExecute(Sender: TObject);
var i    :Integer;
begin
  PrimeFields.Clear;
  NoSQLFields.Clear;
...
procedure TKylixWorkBenchForm.pc_mainChange(Sender: TObject);
begin
  pc_main.ActivePage:=ts_sql;
end;
