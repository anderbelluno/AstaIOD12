...
type
  ...
  private
    ...
    function GetActiveRecordBuffer: PChar;overload;
  ...
  public
    ...
    procedure LoadFromStringWithFields(S: string);
...
function DataSetToStringwithFieldProperties(D: TAstaIODataSet): string;
function DataSetToString(D: TAstaIODataSet): string;
function StringToDataSet(S: string): TAstaIODataSet;
function StringToDataSetwithFieldProperties(S: string): TAstaIODataSet;
function CloneDataSetToString(D: TDataSet): string;overload;
function CloneDataSetToString(D: TDataSet;RowsToReturn:Integer): string;overload;

procedure PopulateDataSetFromStringWithFieldProperties(DS: TAstaIODataSet; const S: string);
...
begin
  ...
  if (FieldCount > 0) and (Fields.Count = 0) then
...
begin
  ...
  if not FDirectWrite then begin
...
begin
  ...
  DecPos := Pos(DecimalSeparator, TempString);
  if (DecPos > 0) then
  begin
...
function TAstaIOCustomDataset.AllocRecordBuffer: PChar;
begin
  GetMem(Result, FRecBufSize);
  Fillchar(result^, FRecbufSize, #0);
end;

procedure TAstaIOCustomDataset.FreeRecordBuffer(var Buffer: PChar);
begin
...
procedure TAstaIOCustomDataset.InternalInitRecord(Buffer: PChar);
begin
...
begin
  ...
  item.GetFromBuffer(ActiveBuffer);
  try
    if State <> dsEdit then begin
      FIndexes.RecordInserted(item);
      FAggregates.RecordInserted(item);
...
function TAstaIOCustomDataset.GetRecord(Buffer: PChar; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
var
  Acceptable: Boolean;
...
procedure TAstaIOCustomDataset.GetBookmarkData(Buffer: PChar;
  Data: Pointer);
begin
...
begin
  ...
  if BookmarkAvailable then begin
    ...
    GetBookmarkData(FFilterBuffer, @Result)
  else if state= dsInsert then result:=FlastBookMark
end;

procedure TAstaIOCustomDataset.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  PRecInfo(Buffer + FRecInfoOfs).Bookmark := PLongInt(Data)^;
end;

function TAstaIOCustomDataset.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer + FRecInfoOfs).BookmarkFlag;
end;

procedure TAstaIOCustomDataset.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PRecInfo(Buffer + FRecInfoOfs).BookmarkFlag := Value;
end;

procedure TAstaIOCustomDataset.InternalSetToRecord(Buffer: PChar);
begin
  ...
  try
    ...
    if buffer = nil then begin
      ...
      if (Field.FieldKind in [fkCalculated, fkLookup]) then
       ...
       //returning Not Null for calc field as the get changed flag only works for normal fields
    ...
    case Field.DataType of
      ftString,
        ftFixedChar,
        ftguid      : StrLCopy(Buffer, FieldAddrInBuffer, FieldSizeFromField(Field));
      ftwidestring,
        ftvarbytes,
...
begin
  ...
  begin
    ...
    if Boolean(Destinationbuffer[0]) then begin
      Pc := @Destinationbuffer[1];
  ...
  if buffer = nil then
    ...
    IsNull := False;
      // Character fields have buffer assigned, but length = 0
    //if ((Field.DataType in [ftString, ftWideString, ftFixedChar]) and (StrLen(Buffer) = 0)) then       //xx
    if (Field.DataType in [ftString, ftFixedChar]) and
       (StrLen(Buffer) = 0) then
      isNull := True
    else if (Field.DataType = ftWideString) and (WideStrLen(PWideChar(Buffer), Field.Size) = 0) then
      isNull := True
  end;
  if field <> nil then
...
begin
  if Field.DataType = ftBCD then
    Currency(Dest^) := Currency(Source^)
     ...
     {$ifdef WideStrChange}
     if ToNative then
          WideStrLCopy(WideString(Source^), PWideChar(Dest), Field.Size)
        else
          WideStrLSet(WideString(Dest^), PWideChar(Source), Field.Size)
    {$else}
    if ToNative then begin
      PWord(Dest)^ := Length(WideString(Source^));
      Move(WideString(Source^)[1], (PChar(Dest) + sizeof(Word))^, Length(WideString(Source^)) * sizeof(WideChar));
    end
    else
      SetString(WideString(Dest^), PWideChar(PChar(Source) + sizeof(Word)), PWord(Source)^);
   {$endif}
  end
  else if (Field.DataType = ftDateTime) and not ToNative and (Double(Source^) = 0) then begin
    TimeStamp.Time := 0;
...
begin
  ...
  if MasterFields = '' then
    Exit;
  i := 0;
  tray := tokencount(MasterFields + ';', i, ';');
  while tray <> '' do begin
    Result := AnsiCompareText(FDataLink.DataSet.FieldByName(Tray).AsString,
      FieldbyName(tokencount(FDetailFields + ';', i, ';')).AsString) = 0;
    if not Result then
      Exit;
    inc(i);
    tray := tokencount(MasterFields + ';', i, ';')
...
function TAstaIOCustomDataset.FilterRecord(Buffer: PChar): Boolean;
var
...
procedure TAstaIOCustomDataset.ClearCalcfields(Buffer: Pchar);
begin
  fillchar(buffer[FStartCalculated], calcfieldssize, #0);
end;

function TAstaIOCustomDataset.GetActiveRecordBuffer: PChar;
begin
  if FDirectRead or FDirectWrite then
    ...
    case State of
      ...
      dsInsert:
        Result := ActiveBuffer;
      dsSetKey:
        Result := PChar(FKeyBuffer) + SizeOf(TAstaIOKeyBuffer);
      else
        Result := nil;
    end;
end;
...
function StreamToString(TM: TMemoryStream): string;
var
  p: Pointer;
begin
...
function TAstaIOCustomDataset.SaveToString: string;
...
procedure TAstaIOCustomDataset.LoadFromString(S: string);
...
procedure TAstaIOCustomDataset.LoadFromStringWithFields(S: string);
var
  TempStream: TMemoryStream;
begin
  TempStream := TMemoryStream.Create;
...
function CloneDataSetToString(D: TDataSet;RowsToReturn:Integer): string;
var
  m: TMemorySTream;
  Ads: TAstaIODataSet;
begin
...
{$endif}
      ...
      AIsNull := not GetChangedFlag(AField, PChar(AKey.FData));
      if not AIsNull then begin
        buff := OffsetPointer(PChar(AKey.FData), FFldOffs^[AField.FieldNo - 1]);
        case AField.DataType of
  ...
  begin
    if AField.FieldKind <> fkData then begin
      ...
      if not AIsNull then begin
        ...
        if sz2 < sz then
          sz := sz2;
        SetString(AValue, PChar(buff), sz);
      end;
    end;
  end;
    ...
    case GetLastError of
      ...
      ERROR_CALL_NOT_IMPLEMENTED:
        begin
          a1 := s1;
          a2 := s2;
          Result := CompareStringA(LOCALE_USER_DEFAULT, flag, PChar(a1), Length(a1),
...
begin
  if not CheckMasterFilter and not Filtered then
    ...
    try
      Result := FilterRecord(TempBuffer);
    finally
      EndDirectRead;
    end;
  end;
...
begin
  ...
  if Length(ARec.FData) = FAstaList.FBufferSize then begin
    Move(ARec.FData[1], TempBuffer^, FAstaList.FBufferSize);
    ...
    Move(ARec.FData[1], TempBuffer^, FRecBufSize)
  else begin
    FDirectRead := False;
...
begin
  ...
  if FDirectWriteRec.IsFake then
    FDirectWriteRec.GetFromBufferExt(TempBuffer, FRecBufSize)
  else
    FDirectWriteRec.GetFromBuffer(TempBuffer);
end;
...
begin
  if FOldValuesDataSet <> nil then
    Result := TAstaIOCustomDataset(FOldValuesDataSet).GetFieldData(Field.FieldNo, Buffer)
  else
    Result := False;
end;
...
begin
  ...
  if not BookmarkValid(@BMInt) then Exit;
